def gauss_jordan(augmented, n):
    for i in range(n):
        # Find pivot and check for zero (singular matrix)
        pivot = augmented[i][i]
        if pivot == 0:
            raise ValueError("Zero pivot encountered. Matrix may be singular or require row swapping.")

        # Normalize pivot row
        augmented[i] = [x / pivot for x in augmented[i]]

        # Elimate colum entries in other rows
        for k in range(n):
            if k != i:
...
def main():
    try:
        ...
        for i in range(n):
            ...
            augmented.append(A[i] + identity + [b[i]])  # i.e A | I | b
